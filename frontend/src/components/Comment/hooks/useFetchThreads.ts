...
export const useFetchThreads = ({
  ...
  // Enable query if documentId exists and either:
  // - Both offsets are undefined (fetch all threads)
  // - Both offsets are defined (fetch filtered threads)
  const enabled =
    !!documentId &&
  ...
  return useQuery({
    ...
    queryFn: ({ queryKey }) => {
      const [, docId, params] = queryKey as [
        string,
        string,
